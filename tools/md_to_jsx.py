...
def parse_module(num: int, title: str, body: str) -> dict:
    ...
    intro = parse_blocks(sub_text)
    # Keep a short card description separately from the complete introduction.
    if sub_text:
    ...
    # selfcheck
    full_practice = re.search(r"### Практика Модуля \d+\n\n?(.*?)(?=\n### Вопросы для самопроверки|\Z)", body, re.DOTALL)
    practice_blocks = parse_blocks(full_practice.group(1)) if full_practice else []
    selfcheck = []
...
def emit_module(mod, indent=2):
    ...
    out.append(" " * (indent + 2) + f'intro: {js_string(mod["intro"])},')
    out.append(" " * (indent + 2) + "sections: [")
    for sec in mod["sections"]:
        out.append(emit_section(sec, indent + 4) + ",")
    out.append(" " * (indent + 2) + "],")
    if mod["practice"]:
        out.append(" " * (indent + 2) + f'practiceBlocks: {js_string(mod["practiceBlocks"])},')
...
def main():
    ...
    preface = text.split('## ВАЖНО ПЕРЕД НАЧАЛОМ', 1)[1].split('## ОГЛАВЛЕНИЕ', 1)[0]
    mods[0]['intro'] = [{'type': 'h', 'text': 'Важно перед началом'}, *parse_blocks(preface), *mods[0]['intro']]
    edition = re.search(r'\*Редакция: ([\d-]+)', text).group(1)
    metadata = {'edition': edition}
    data_dir = REPO / 'dist' / 'course'
    data_dir.mkdir(parents=True, exist_ok=True)
    catalog = []
    search = []
    for mod in mods:
        (data_dir / f'{int(mod["n"])}.json').write_text(js_string({**mod, 'loaded': True}), encoding='utf8')
        catalog.append({k: mod[k] for k in ('n', 'title', 'sub')})
        catalog[-1]['sections'] = [{k: s[k] for k in ('id', 'title')} for s in mod['sections']]
        for section in mod['sections']:
            body = ' '.join(b.get('text', ' '.join(b.get('items', [])) or ' '.join(c for row in b.get('rows', []) for c in row)) for b in section['blocks'])
            search.append({'moduleIdx': int(mod['n']), 'moduleN': mod['n'], 'sectionId': section['id'], 'sectionTitle': section['title'], 'body': body})
        for target, title, blocks in [('module-start', 'Введение к модулю', mod['intro']), ('practice', 'Практика модуля', mod['practiceBlocks']), ('selfcheck', 'Вопросы для самопроверки', [{'type':'list','items':mod['selfcheck']}])]:
            body = ' '.join(b.get('text', ' '.join(b.get('items', [])) or ' '.join(c for row in b.get('rows', []) for c in row)) for b in blocks)
            if body: search.append({'moduleIdx': int(mod['n']), 'moduleN': mod['n'], 'sectionId': None, 'targetId': target, 'sectionTitle': title, 'body': body})
    (data_dir / 'search.json').write_text(js_string(search), encoding='utf8')
    (REPO / 'site' / '_servitors-catalog.jsx').write_text('// Generated by tools/md_to_jsx.py\nwindow.SERVITORS_META = '+js_string(metadata)+';\nwindow.SERVITORS_MODULES = '+js_string(catalog)+';\n', encoding='utf8')
